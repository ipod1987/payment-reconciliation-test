...
There is no idempotency guard in `ReconciliationService`. Every call triggers a new 3-way fetch regardless of whether the same payment was reconciled moments ago. Adding a TTL check (e.g., skip re-reconciling a payment reconciled less than 5 minutes ago) would reduce redundant external calls during burst traffic. To implement: add a `LoadReconciliationResultPort` output port, implement it in `ReconciliationJpaAdapter` using a `findLatestByPaymentId` JPQL query on `ReconciliationJpaRepository`, and add a single conditional at the top of `ReconciliationService.reconcile()` that returns the cached result if it is within the TTL window.
